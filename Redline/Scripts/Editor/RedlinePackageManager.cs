...
namespace Redline.Scripts.Editor {
    public abstract class RedlineImportManager {
        ...
        public static async void DownloadAndImportAssetFromServer(string assetName) {
            var assetPath = Path.Combine(RedlineSettings.GetAssetPath(), assetName);
        ...
        private static async Task DownloadFile(string assetName) {
            ...
            var downloadPath = Path.Combine(RedlineSettings.GetAssetPath(), assetName);
            ...
            if (webRequest.result == UnityWebRequest.Result.Success) {
	            ...
	            await File.WriteAllBytesAsync(downloadPath, webRequest.downloadHandler.data);
        ...
        public static async void UpdateConfig() {
            const string url = InternalServerUrl + ConfigName;
            var updatePath = Path.Combine(RedlineSettings.ProjectConfigPath, "update_" + ConfigName);
            ...
            if (webRequest.result == UnityWebRequest.Result.Success) {
	            // Ensure directory exists before writing
	            Directory.CreateDirectory(Path.GetDirectoryName(updatePath) ?? string.Empty);
	            await File.WriteAllBytesAsync(updatePath, webRequest.downloadHandler.data);
	            RedlineLog("Config download completed!");

	            // Replace old config file
	            var oldConfigPath = Path.Combine(RedlineSettings.ProjectConfigPath, ConfigName);
