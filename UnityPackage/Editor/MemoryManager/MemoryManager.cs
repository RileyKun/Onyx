...
namespace Redline.Editor.MemoryManager
{
	...
	public class MemoryManagementTool : EditorWindow
	{
		...
		private static readonly Dictionary<MemoryPreset, float> MemoryPresets = new()
		{
			{MemoryPreset.VeryLowEnd, 1024},    // 1GB
			{MemoryPreset.LowEnd, 2048},        // 2GB
			{MemoryPreset.Average, 4096},       // 4GB
			{MemoryPreset.HighEnd, 8192},       // 8GB
			{MemoryPreset.AutoPhysical, 0},     // Will be set based on system memory
			{MemoryPreset.AutoTotal, 0},        // Will be set based on total memory
			{MemoryPreset.Custom, 0}            // Will be set by user
		};
		private static MemoryPreset _selectedPreset = MemoryPreset.AutoPhysical;
		...
		{
			// Load saved preset
			var savedPreset = EditorPrefs.GetString(PrefSelectedPreset, MemoryPreset.AutoPhysical.GetDescription());
			_selectedPreset = MemoryPresetExtensions.FromDescription(savedPreset);

			// Load saved threshold
			_memoryThresholdMb = _selectedPreset == MemoryPreset.Custom ? EditorPrefs.GetFloat(PrefCustomMemory, _memoryThresholdMb) : MemoryPresets[_selectedPreset];
		...
		{
			EditorPrefs.SetString(PrefSelectedPreset, _selectedPreset.GetDescription());
			if (_selectedPreset == MemoryPreset.Custom)
			{
		...
		private static void DetectSystemMemory()
		{
			try
			{
				...
				// Update preset values based on detected memory
				UpdatePresetValues();
			...
			catch (Exception e)
			{
				...
				// Update preset values with fallback values
				UpdatePresetValues();

				if (_selectedPreset is MemoryPreset.AutoPhysical or MemoryPreset.AutoTotal)
				{
		...
		private void OnGUI()
		{
			...
			if (_showMemoryLimits)
			{
				...
				// Get all preset descriptions for the dropdown
				var presetDescriptions = Enum.GetValues(typeof(MemoryPreset))
					.Cast<MemoryPreset>()
					.Select(p => p.GetDescription())
					.ToArray();

				// Find the current preset's index
				var currentPresetIndex = Array.IndexOf(presetDescriptions, _selectedPreset.GetDescription());
				
				// Show the dropdown with descriptions
				var newPresetIndex = EditorGUILayout.Popup("Memory Preset", currentPresetIndex, presetDescriptions);
				
				if (newPresetIndex != currentPresetIndex)
				{
					_selectedPreset = (MemoryPreset)newPresetIndex;
					if (_selectedPreset != MemoryPreset.Custom)
					{
			...
			if (_showAutoCleanup)
			{
				...
				// Fix layout for timer-based cleanup toggle
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.LabelField("Enable Timer-Based Cleanup", GUILayout.Width(200));
				var newAutoCleanup = EditorGUILayout.Toggle(_autoCleanupEnabled);
				EditorGUILayout.EndHorizontal();
				...
				// Fix layout for cleanup interval
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.LabelField("Cleanup Interval (seconds)", GUILayout.Width(200));
				var newInterval = EditorGUILayout.FloatField(_cleanupIntervalSeconds);
				EditorGUILayout.EndHorizontal();
		...
		private static void UpdatePresetValues()
		{
			// Update auto presets based on detected system memory
			MemoryPresets[MemoryPreset.AutoPhysical] = _totalPhysicalMemoryMb / 2;
			MemoryPresets[MemoryPreset.AutoTotal] = _totalSystemMemoryMb / 2;

			// If current preset is auto, update the threshold
			if (_selectedPreset == MemoryPreset.AutoPhysical || _selectedPreset == MemoryPreset.AutoTotal)
			{
				_memoryThresholdMb = MemoryPresets[_selectedPreset];
				SaveSettings();
			}
